...
export const validators = {
  ...
  range: (min: number, max: number, errorMsg?: string): CustomValidatorFn => (value) => {
    if (value === undefined) return true
    if (!DECIMAL_RE.test(value)) return errorMsg || `Value must be between ${min} and ${max}`
  ...
  awsArn: (service?: string, errorMsg?: string): CustomValidatorFn => (value) => {
    if (value === undefined) return true
    const arnPattern = /^arn:(aws|aws-cn|aws-us-gov):[a-z0-9-]+:[a-z0-9-]*:\d{12}:.+$/
  ...
  all: (...fns: CustomValidatorFn[]): CustomValidatorFn => {
    const validFns = fns.filter((fn): fn is CustomValidatorFn => typeof fn === 'function')
    return (value) => {
